...
function findElementIndexes(arr, element) {
  ...
  if (!Array.isArray(arr)) {
    return []; // Повертаємо пустий масив, якщо аргумент не є масивом
  }
  ...
  if (firstIndex === -1) {
    return []; // Повертаємо пустий масив, якщо елемент не знайдено
  }

  // Знаходимо індекс останнього знайденого елементу за допомогою методу `lastIndexOf`
  var lastIndex = arr.lastIndexOf(element);

  // Створюємо масив індексів знайдених елементів
  var indexes = [];

  // Додаємо перший знайдений індекс
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (
    ...
    return arr; // Повертаємо вхідний масив без змін
  }
...
function sortByKey(arr, key) {
  // Використовуємо метод `sort` і передаємо в нього функцію порівняння для сортування масиву об'єктів за заданим ключем
...
function customAt(arr, index) {
  ...
  // Перевіряємо тип отриманого елементу та виводимо відповідне повідомлення
...
function customIncludes(arr, element) {
  ...
  // Виводимо кількість зустрічей елемента в консоль
  console.log(`Елемент ${element} зустрічається ${count} раз(ів)`);
